/**
 * Universal Document API — HTTP entry point.
...
import { parseDocumentRequest, type Document, type DocumentRequest } from "./schema.js";
...
type DocumentJob = Job<DocumentRequest, Document>;
const store = new InMemoryStore<DocumentJob>();
...
queue.process(async (id) => {
  ...
  try {
    job.output = await processDocument(job.input);
    job.status = "done";
  } catch (e) {
    job.status = "failed";
...
const MANIFEST = {
  ...
  endpoints: [
    { method: "POST", path: "/document", summary: "Submit a document for parsing." },
    { method: "GET", path: "/document/:id", summary: "Fetch job status and result." },
...
server.listen(PORT, () => {
  ...
  console.log(`klaro26 document api → http://localhost:${PORT}`);
});
