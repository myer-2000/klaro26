...
import { parseResearchRequest, type ResearchRequest, type ResearchResult } from "./schema.js";
import { processResearch } from "./pipeline.js";

const PORT = Number(process.env.PORT ?? 8084);
const KEYS = loadKeys();
const LIMITER = new TokenBucket({ rpm: 300, burst: 30 });

type ResearchJob = Job<ResearchRequest, ResearchResult>;
const store = new InMemoryStore<ResearchJob>();
...
queue.process(async (id) => {
  ...
  try {
    job.output = await processResearch(job.input);
    job.status = "done";
  } catch (e) {
    job.status = "failed";
...
const MANIFEST = {
  ...
  endpoints: [
    { method: "POST", path: "/research", summary: "Run a research query." },
    { method: "GET", path: "/research/:id", summary: "Fetch job status and result." },
...
server.listen(PORT, () => {
  ...
  console.log(`klaro26 research api → http://localhost:${PORT}`);
});
